...
namespace MachineLearningGames.Alihan.Validation.FluentValidation
{
    public class DataValidator : AbstractValidator<Customer>
    {
        ...
        {
            RuleFor(x => x.Age).NotEmpty().WithMessage("Yaş bilgisi boş olmamalı").GreaterThan(0)
            ...
            RuleFor(x => x.ParentOrChildren).NotEmpty().WithMessage("Lütfen ebeveyn ya da çocuk bilgisini seçiniz").GreaterThan(-1).WithMessage("Lütfen ebeveyn ya da çocuk sayısını giriniz").NotNull().WithMessage("Lütfen ebeveyn ya da çocuk sayısını boş bırakmayınız");
            RuleFor(x => x.SiblingsOrSpouse).NotEmpty().WithMessage("Kardeş ya da eş sayısı bilgisi boş olmamalı").GreaterThan(-1)
            ...
            RuleFor(x => x.TicketFare).NotEmpty().WithMessage("Bilet ücreti bilgisi boş olmamalı").GreaterThan(-1)
